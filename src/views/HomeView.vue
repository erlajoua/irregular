...
<template>
  <div class="w-screen h-screen bg-primary flex pb-12 md:pb-0 md:items-center justify-center">
    <div class="w-full mt-12 md:mt-0 mx-6 md:mx-0  md:w-8/12 md:h-[80vh] rounded-2xl bg-primary_2 relative">
      <!-- <ChangeLanguage /> -->
...
<style scoped>
  ...
  .icon {
    ...
    width: var(--size);
    height: var(--size);
    transform: translate(-50%, calc(-50% + (50% - var(--size) / 2)));
  }

  .titleapp {
    left: 50%;
    top: 0%;
    transform: translate(-50%, calc(-50% + (50% + 1.2em)));
  }
</style>
